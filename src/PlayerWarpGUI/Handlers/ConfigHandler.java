...
public class ConfigHandler {
	...
	public ConfigHandler(PlayerWarpGUI pl) {
		ConfigHandler.p = pl;
	...
	public void setUp() {
		File config = new File(p.getPathConfig());
		if (!config.exists()) {
	...
	public void setupDefaults() {
		...
		setDefaultValues("General.Locale", "en_us", "Available: en_us, en_au");
	...
	private void setDefaultValues(String key, Object def, String comment) {
		if (def != null) {
			p.getConfig().set(key, p.getConfig().get(key, def));
		}
		if (comment != null) {
	...
	public void saveConfig() {
		...
		p.getConfig().options().header(null);
		...
		for (String configLine : p.getConfig().getKeys(true)) {
			String[] configKey = configLine.split("\\" + p.getConfig().options().pathSeparator());
			...
			Object configValue = p.getConfig().get(configLine);
			if (!p.getConfig().isConfigurationSection(configLine)) {
		...
		try {
			Files.write(sb, new File(p.getPathMain(), "config.yml"), Charsets.UTF_8);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
